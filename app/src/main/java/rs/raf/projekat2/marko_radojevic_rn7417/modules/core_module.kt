...
//definisemo promenljivu coreModule, module je funkcija,
val coreModule = module {

    //single je funkcija  koja pravi komponentu SharedPreferences koja je singleton i sad mozemo to da inject() gde god hocemo
    single<SharedPreferences> {
        androidApplication().getSharedPreferences(androidApplication().packageName, Context.MODE_PRIVATE)
    }

    single { Room.databaseBuilder(androidContext(), BeleskaDatabase::class.java,"BeleskaDb")
        .fallbackToDestructiveMigration().build()
    }

    single { Room.databaseBuilder(androidContext(), PredmetDataBase::class.java,"PredmetDb")
        .fallbackToDestructiveMigration().build()
    }

    //pravimo komponentu Moshi (optimizovaniji JSON) i nju sad mozemo da inject()
    single { createMoshi() }
